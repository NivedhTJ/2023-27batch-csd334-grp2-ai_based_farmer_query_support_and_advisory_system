...
async function loadSidebar() {
    ...
    try {
        ...
        if (window.showToast) showToast("Failed to load sidebar", "error");
    }
}
...
async function loadSessionHistory(sid) {
    ...
    try {
        ...
        if (window.showToast) showToast("Failed to load chat history", "error");
    }
}
...
async function clearChat() {
    ...
    try {
        ...
        if (response.ok) {
            if (window.showToast) showToast("Chat history cleared.", "success");
            startNewChat();
        }

    } catch (err) {
        console.error(err);
        if (window.showToast) showToast("Failed to clear chat history.", "error");
    }
}
